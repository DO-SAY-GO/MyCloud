  ...
  async moveCopy(req, res, r, url) {
    ...
    const work = async () => {
      ...
      try {
        ...
        else if (existed && dest.collection) await this.store.deleteCollection(dest.fsPath);
    ...
    if (!this.limits) await work();
      ...
      if (move && from === to) {
        await this.limits.withBytes(to, 0, 0, work);
        if (replacing) this.limits.freed(to, { bytes: replacing + ENTRY_COST, entries: 1 }); // the replaced object is gone
      } else {
        await this.limits.withBytes(to, stats.bytes, replacing, work, { entries: replacing ? stats.entries - 1 : stats.entries });
