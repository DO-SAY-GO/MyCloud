...
import { Limits, treeStats, ENTRY_COST } from './limits.js';
...
export class Dav {
  constructor(store, { limits, activity, drive } = {}) {
    this.store = store;
    this.limits = limits ?? new Limits(store, {});
    this.drive = drive ?? new Drive(store, this.limits); // every Drive write (shared with the web API)
  ...
  async put(req, res, r) {
    ...
    if (r.area === 'files') {
      ...
      ({ replaced: replacedNow } = await this.drive.put(r.user, r.fsPath, req));
    } else {
      ...
      await this.admit(r.user, r.collDir, body.length, r.stat?.size ?? 0,
        ...
        } }), { entries: existed ? 0 : 1, peakEntries: 1 });
    }
    const headers = {};
  ...
  async delete(req, res, r) {
    ...
    if (r.area === 'files' && r.rest.length > 0) {
      ...
      await this.drive.trash(r.user, r.fsPath, r.rest.join('/'));
  ...
  async mkcol(req, res, r) {
    ...
    if (r.area === 'files') {
      ...
      const parent = await statOrNull(path.dirname(r.fsPath));
      if (!parent?.isDirectory()) throw new HttpError(409, 'parent does not exist');
      await this.drive.mkdir(r.user, r.fsPath);
  ...
  async moveCopy(req, res, r, url) {
    ...
    if (isItem(r)) {
      ...
      // Drive items: the shared Drive path (budget and path locks, metered copies, measured moves).
      ({ replaced: replacedNow } = await this.drive.transfer({ user: r.user, move, src: r.fsPath, dest: dest.fsPath, destRel: dest.rest.join('/'), overwrite }));
    }
    if (isItem(r) && move && crossBudget) this.limits.freed(from, { bytes: moved.bytes + moved.entries * ENTRY_COST, entries: moved.entries });
  ...
  async lock(req, res, r) {
    ...
    if (r.kind === 'missing' && r.area === 'files' && r.rest.length) {
      ...
      await this.drive.lockCreate(r.user, r.fsPath);
      status = 201;
    }
