...
export class Dav {
  ...
  async put(req, res, r) {
    ...
    if (r.area === 'files') {
      ...
      await this.admit(r.user, r.collDir, body.length, r.stat?.size ?? 0,
        (res) => this.store.writeItem(r.collDir, r.rest[1], body, { settle: res && ((x) => this.limits.settle(res, x)) }), { entries: existed ? 0 : 1 });
    }
    const headers = {};
  ...
  async moveCopy(req, res, r, url) {
    ...
    const work = async () => {
      ...
      let trashed = null;
      try {
        // What's at the destination *now* decides (something may have appeared since the request was checked).
        const now = await fs.lstat(dest.fsPath).catch(() => null);
        if (now && dest.area === 'files') trashed = await this.store.trash(r.user, dest.fsPath, dest.rest.join('/'));
        else if (now?.isDirectory()) await this.store.deleteCollection(dest.fsPath);
    ...
    // Calendar/contact objects: the content change and both collections' sync logs commit together.
    const changes = isItem(r) ? [...(move ? [{ dir: r.collDir, name: r.rest[1], deleted: true }] : []), { dir: dest.collDir, name: dest.rest[1] }] : null;
    const plain = work;
    const unit = changes ? () => this.store.withSyncChanges(changes, plain) : plain;
    if (!this.limits) await unit();
    else {
      ...
      if (move && from === to) {
        await this.limits.withBytes(to, 0, 0, unit);
        if (replacing) this.limits.freed(to, { bytes: replacing + ENTRY_COST, entries: 1 }); // the replaced object is gone
      } else {
        await this.limits.withBytes(to, stats.bytes, replacing, unit, { entries: replacing ? stats.entries - 1 : stats.entries });
        if (move) this.limits.freed(from, { bytes: stats.bytes + stats.entries * ENTRY_COST, entries: stats.entries }); // left `from`
      }
    }
