...
export class Dav {
  constructor(store, { limits, activity } = {}) {
    ...
    this.hooks = {}; // test seams (beforeStage, beforeMoveLock): race windows between sizing and the locked commit
  }
  ...
  async moveCopy(req, res, r, url) {
    ...
    let moved = stats; // what a cross-budget move really moved (re-measured under its locks)
    ...
    if (isItem(r)) {
      ...
      if (move && !crossBudget) {
        ...
        const { replaced, size } = await admit(stats.bytes, 0, transfer, 1);
        replacedNow = replaced !== null;
        moved = { bytes: size, entries: 1 };
      }
    } else {
      ...
      const work = move
        ? async (res) => { await this.hooks.beforeMoveLock?.(r.fsPath); return this.store.withLocks([r.fsPath, dest.fsPath], async () => {
          if (!(await fs.lstat(r.fsPath).catch(() => null))) throw Object.assign(new Error('source vanished'), { code: 'ENOENT' });
          // Into another budget: bill what is really moving, measured now, under the locks.
          if (res && crossBudget) {
            moved = await treeStats(r.fsPath);
            this.limits.settle(res, { bytes: moved.bytes, entries: moved.entries });
          }
        ...
        : async (res) => {
          try {
            await this.hooks.beforeStage?.(r.fsPath);
            // Every byte copied passes the meter: the copy is charged for what it really copies, and quota and disk
            // floors hold during the copy itself, even if the source grew after it was first measured.
            if (res) {
              await meteredCopy(r.fsPath, stage, (n) => this.limits.consume(res, n), () => this.limits.claimEntry(res));
              this.limits.settle(res, { entries: res.usedEntries ?? 0 }); // exactly what was created (may be fewer)
            } else await fs.cp(r.fsPath, stage, { recursive: true, filter: async (src) => !(await fs.lstat(src)).isSymbolicLink() });
    ...
    if (move && crossBudget && this.limits) this.limits.freed(from, { bytes: moved.bytes + moved.entries * ENTRY_COST, entries: moved.entries });
