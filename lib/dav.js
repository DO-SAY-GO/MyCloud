...
export class Dav {
  ...
  async put(req, res, r) {
    ...
    if (r.area === 'files') {
      ...
      try {
        ...
        if (this.limits) await this.limits.guard(await this.limits.budgetFor(r.user, r.fsPath), req, r.stat?.size ?? 0, (meter) => write(meter));
        else await write();
      ...
      await this.admit(r.user, r.collDir, body.length, r.stat?.size ?? 0, () => this.store.writeItem(r.collDir, r.rest[1], body));
    }
    const headers = {};
  ...
  async mkcol(req, res, r) {
    ...
    if (r.area === 'files') {
      ...
      const parent = await statOrNull(path.dirname(r.fsPath));
      if (!parent?.isDirectory()) throw new HttpError(409, 'parent does not exist');
      await this.admit(r.user, path.dirname(r.fsPath), 4096, 0, () => fs.mkdir(r.fsPath));
      ...
      await this.admit(r.user, path.dirname(r.fsPath), 4096 + JSON.stringify(props).length, 0, () => this.store.createCollection(r.user, r.area, r.rest[0], props));
    } else {
  ...
  async moveCopy(req, res, r, url) {
    ...
    if (existed) {
      if (dest.area === 'files') await this.store.trash(r.user, dest.fsPath, dest.rest.join('/'));
      else await fs.rm(dest.fsPath, { recursive: true });
    }
    if (req.method === 'MOVE') {
      // A move within one budget adds nothing; into another budget (e.g. out of Family) it bills the destination.
      const from = this.limits && (await this.limits.budgetFor(r.user, r.fsPath));
      const to = this.limits && (await this.limits.budgetFor(r.user, path.dirname(dest.fsPath)));
      if (from === to) await fs.rename(r.fsPath, dest.fsPath);
      else await this.admit(r.user, path.dirname(dest.fsPath), await treeSize(r.fsPath), 0, () => fs.rename(r.fsPath, dest.fsPath));
    } else {
      // Sized (recursively) and admitted before a single byte is copied.
      await this.admit(r.user, path.dirname(dest.fsPath), await treeSize(r.fsPath), 0, async () => {
        try {
          await fs.cp(r.fsPath, dest.fsPath, { recursive: true, filter: async (src) => !(await fs.lstat(src)).isSymbolicLink() });
        } catch (e) {
          await fs.rm(dest.fsPath, { recursive: true, force: true });
          throw e;
        }
      });
    }
