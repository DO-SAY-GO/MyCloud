...
export class Dav {
  ...
  async put(req, res, r) {
    ...
    if (r.area === 'files') {
      ...
      try {
        ...
        if (this.limits) await this.limits.guard(r.user, req, r.stat?.size ?? 0, (meter) => write(meter));
        else await write();
  ...
  async moveCopy(req, res, r, url) {
    ...
    else await fs.cp(r.fsPath, dest.fsPath, { recursive: true, filter: async (src) => !(await fs.lstat(src)).isSymbolicLink() });
    if (isItem(r)) {
