...
import { treeStats } from './limits.js';
...
export class Dav {
  ...
  async proppatch(req, res, r) {
    ...
    // The rewritten file is sized generously (old + everything set) and passes the storage gate like any write.
    if (r.kind === 'calendar' || r.kind === 'addressbook') {
      const old = (await statOrNull(path.join(r.fsPath, '.props.json')))?.size ?? 0;
      await this.admit(r.user, r.fsPath, old + JSON.stringify(set).length, old, () => this.store.patchProps(r.fsPath, set, remove));
    }
  ...
  async put(req, res, r) {
    ...
    if (r.area === 'files') {
      ...
      try {
        ...
        if (this.limits) await this.limits.guard(await this.limits.budgetFor(r.user, r.fsPath), req, r.stat?.size ?? 0, (meter) => write(meter), { entries: existed ? 0 : 1 });
        else await write();
      ...
      await this.admit(r.user, r.collDir, body.length, r.stat?.size ?? 0, () => this.store.writeItem(r.collDir, r.rest[1], body), { entries: existed ? 0 : 1 });
    }
    const headers = {};
  ...
  async mkcol(req, res, r) {
    ...
    if (r.area === 'files') {
      ...
      await this.admit(r.user, path.dirname(r.fsPath), 4096 + JSON.stringify(props).length, 0, () => this.store.createCollection(r.user, r.area, r.rest[0], props), { entries: 3 });
    } else {
  ...
  async moveCopy(req, res, r, url) {
    ...
    // Nothing at the destination is touched until the operation is admitted and fully staged beside it; the swap
    // is then a rename. A replaced Drive item goes to Recently Deleted (and comes back if the swap fails).
    const move = req.method === 'MOVE';
    const stats = await treeStats(r.fsPath);
    const destDir = path.dirname(dest.fsPath);
    const stage = path.join(destDir, `.mycloud-stage-${crypto.randomBytes(6).toString('hex')}`);
    // Overwriting a calendar/contact object replaces it (its bytes are freed); a Drive destination is kept in trash.
    const replacing = existed && dest.area !== 'files' && !dest.collection ? dest.stat?.size ?? 0 : 0;
    const work = async () => {
      if (move) await fs.rename(r.fsPath, stage);
      else {
        try {
      ...
      let trashed = null;
      try {
        if (existed && dest.area === 'files') trashed = await this.store.trash(r.user, dest.fsPath, dest.rest.join('/'));
        else if (existed && dest.collection) await fs.rm(dest.fsPath, { recursive: true });
        await fs.rename(stage, dest.fsPath); // a single calendar/contact object is replaced atomically here
      } catch (e) {
        if (move) await fs.rename(stage, r.fsPath).catch(() => {});
    ...
    if (!this.limits) await work();
    else {
      // A move within one budget adds nothing; a copy, or a move into another budget, bills the destination.
      const from = await this.limits.budgetFor(r.user, r.fsPath);
      const to = await this.limits.budgetFor(r.user, destDir);
      if (move && from === to) await this.limits.withBytes(to, 0, 0, work);
      else await this.limits.withBytes(to, stats.bytes, replacing, work, { entries: replacing ? stats.entries - 1 : stats.entries });
    }
    if (isItem(r)) {
