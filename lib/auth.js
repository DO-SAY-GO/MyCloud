...
// Password checks are deliberately slow (scrypt), so they are rationed: at most 2 in flight per client address and
// 4 overall, with a bounded queue. Throttling is re-checked after a slot is granted, so a burst of parallel guesses
// is held to the same limits as sequential ones.
class Gate {
  constructor(total = 4, perIp = 2, queue = 64) {
    Object.assign(this, { total, perIp, queueMax: queue, running: 0, byIp: new Map(), queue: [] });
  }
  async run(ip, fn) {
    if ((this.byIp.get(ip) ?? 0) >= this.perIp) throw new HttpError(429, 'too many attempts at once, slow down');
    if (this.queue.length >= this.queueMax) throw new HttpError(503, 'server busy, try again');
    this.byIp.set(ip, (this.byIp.get(ip) ?? 0) + 1);
    try {
      if (this.running >= this.total) await new Promise((r) => this.queue.push(r));
      this.running++;
      try {
        return await fn();
      } finally {
      ...
      const n = this.byIp.get(ip) - 1;
      if (n) this.byIp.set(ip, n);
...
export class Auth {
  ...
  // Verify a password for a sign-in attempt: rationed, throttled, and recorded. Returns true/false.
  async attempt(ip, account, check) {
    if (this.throttled(ip, account)) throw new HttpError(429, 'too many attempts, try again shortly');
    return this.gate.run(ip, async () => {
      if (this.throttled(ip, account)) throw new HttpError(429, 'too many attempts, try again shortly');
      const ok = await check();
      if (ok) this.noteSuccess(ip, account);
      else this.noteFailure(ip, account);
      return ok;
    });
  }
  ...
  async basicUser(req, ip) {
    ...
    let ok;
    try {
      ok = await this.attempt(ip, username, () => this.checkPassword(username, decoded.slice(i + 1), { account: this.davAccountPassword, appPasswords: true }));
    } catch {
      return null; // throttled or busy: the client sees 401 and retries later
    }
    if (!ok) return null;
