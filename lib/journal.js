...
export class Journal {
  ...
  async run(ops) {
    ...
    try {
      ...
      if (e.crash) throw e;
      const failures = await Journal.undo(plan, (op) => op.step, this.fault);
      if (failures.length) {
        // Never throw away the only record that can put this right: startup recovery will finish the rollback.
        e.rollbackIncomplete = failures;
        throw e;
      }
      await syncDirs(Journal.touched(plan));
      // Durably mark it aborted before deleting it: if a crash resurrected a still-"prepared" record, recovery could
      // mistake the restored originals for newly applied content.
