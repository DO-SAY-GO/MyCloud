...
export class Store {
  ...
  async removeCounted(p) {
    ...
    if (!st) return;
    const inner = st.isDirectory() ? await accounted(p) : { bytes: 0, entries: 0 };
  ...
  async createCollection(u, type, id, props = {}) {
    ...
    await this.writeMeta(path.join(dir, '.sync.json'), { seq: 1, log: [] });
    return dir;
  }
  ...
  async recordChange(dir, name, deleted = false) {
    return this.withLock(dir, async () => {
      ...
      await this.writeMeta(path.join(dir, '.sync.json'), state);
      return state.seq;
    });
  }
