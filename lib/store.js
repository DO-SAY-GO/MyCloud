...
export class Store {
  ...
  async withSyncChanges(changes, build) {
  ...
  // Returns the size of the object it replaced (null if none).
  async transferItem({ move, from, fromDir, toDir, toName, overwrite, settle }) {
    ...
    await this.withSyncChanges(changes, async () => {
      if (!(await statOrNull(from))) throw Object.assign(new Error('source vanished'), { code: 'ENOENT' });
      const now = await statOrNull(target);
      if (now && !overwrite) throw new HttpError(412, 'destination exists');
      replaced = now ? now.size : null;
      settle?.({ replacing: now?.size ?? 0, entries: now ? 0 : 1 });
      if (move) return { ops: [{ kind: 'move', from, target }] };
      return { ops: [{ kind: 'put', tmp: await this.stage(toDir, await fs.readFile(from)), target }] };
    });
    return replaced;
  }
