...
// A readers-writer lock, first come first served (a waiting writer holds back later readers, so it can't starve).
class RWLock {
  constructor() {
    this.state = 0; // >0 readers, -1 a writer
    this.waiting = [];
  }
  acquire(exclusive) {
  ...
  release(exclusive) {
    this.state = exclusive ? 0 : this.state - 1;
    this.pump();
  }
  pump() {
    while (this.waiting.length) {
      const next = this.waiting[0];
      if (next.exclusive ? this.state !== 0 : this.state < 0) return;
      this.waiting.shift();
      this.state = next.exclusive ? -1 : this.state + 1;
      next.grant();
...
export class Store {
  constructor(dataDir) {
    ...
    this.budgetLocks = new Map(); // budget root -> RWLock
  }
  ...
  async writeMeta(p, obj) {
    ...
    await this.gate(Buffer.byteLength(data), old ?? 0, old === undefined ? 1 : 0,
      () => writeFileAtomic(p, data), { peakEntries: 1 });
  }
  ...
  async sweep(users) {
    ...
    for (const root of [...users.map((u) => this.userRoot(u)), this.familyRoot()]) {
      for (const name of await fs.readdir(path.join(root, '.staging')).catch(() => [])) {
        const p = path.join(root, '.staging', name);
        if (Date.now() - ((await statOrNull(p))?.mtimeMs ?? 0) > 86400 * 1000) await fs.rm(p, { recursive: true, force: true });
      }
    }
  ...
  // The budget a path belongs to, as its real root folder: the Family space, or users/<name>.
  async budgetRootOf(p) {
    let real = null;
    for (let d = p; !real; d = path.dirname(d)) {
      real = await fs.realpath(d).catch(() => null);
      if (d === path.dirname(d)) break;
    }
    const family = await fs.realpath(this.familyRoot()).catch(() => this.familyRoot());
    if (real && (real === family || real.startsWith(family + path.sep))) return family;
    const users = await fs.realpath(path.join(this.dataDir, 'users')).catch(() => path.join(this.dataDir, 'users'));
    const rel = real ? path.relative(users, real).split(path.sep) : [];
    return rel[0] && rel[0] !== '..' ? path.join(users, rel[0]) : users;
  }

  // Hold the budget locks of several paths, shared or exclusive, always in the same (sorted) order.
  async withBudgets(paths, exclusive, fn) {
    const roots = [...new Set(await Promise.all(paths.map((p) => this.budgetRootOf(p))))].sort();
    const held = [];
    try {
      for (const root of roots) {
        if (!this.budgetLocks.has(root)) this.budgetLocks.set(root, new RWLock());
        await this.budgetLocks.get(root).acquire(exclusive);
        held.push(root);
      }
      return await fn();
    } finally {
      for (const root of held.reverse()) this.budgetLocks.get(root).release(exclusive);
    }
  }
