...
export async function accounted(root, skip = null, base = '') {
  ...
  const walk = async (dir) => {
    for (const e of await fs.readdir(dir, { withFileTypes: true }).catch(() => [])) {
      ...
      if (e.isDirectory() && e.name === '.staging' && dir === root && !base) continue; // in flight: held by reservations
...
export class Limits {
  ...
  constructor(store, env = process.env, { statfs = (p) => fs.statfs(p) } = {}) {
    ...
    this.active = new Set(); // live reservations: quota entries plus peakEntries needed physically while staging
  ...
  // Declared-but-not-yet-written bytes and peak filesystem entries other operations have already claimed. A staged
  // overwrite needs a temporary inode even though it adds no persistent/quota entry, hence the separate peak count.
  pending() {
    let n = 0;
    for (const r of this.active) n += Math.max(0, r.declared - r.bytes) + r.peakEntries * ENTRY_COST;
    return n;
  }
  ...
  availableInodes() {
    ...
    for (const r of this.active) pendingEntries += r.peakEntries;
    return this.measured.inodes - this.entryGrowth - pendingEntries;
  }

  // ---- reservations ---------------------------------------------------------------------------------------------
  // Claim `declared` bytes and `entries` persistent quota entries for `budget` before writing. `peakEntries` is the
  // maximum simultaneous physical entries needed (including temporary staging files) and protects disk/inode floors.
  ...
  async reserve(budget, declared, replacing = 0, entries = 0, peakEntries = entries) {
    entries = Math.max(0, entries);
    peakEntries = Math.max(entries, peakEntries);
    const r = { budget, declared: Number.isFinite(declared) && declared > 0 ? declared : 0, bytes: 0, replacing: Math.max(0, replacing), entries, peakEntries };
    ...
    if (this.available() - r.declared - r.peakEntries * ENTRY_COST < this.reserveBytes) throw full();
    if (this.availableInodes() - r.peakEntries < this.inodeReserve) throw full();
    this.active.add(r);
  ...
  settle(r, { replacing = r.replacing, entries = r.entries, peakEntries = r.peakEntries, bytes } = {}) {
    const before = { net: Limits.net(r), declared: r.declared, entries: r.entries, peakEntries: r.peakEntries };
    r.replacing = Math.max(0, replacing);
    r.entries = Math.max(0, entries);
    r.peakEntries = Math.max(r.entries, peakEntries);
    if (bytes !== undefined) r.declared = Math.max(0, bytes);
    if (r.declared > this.maxUpload) throw new HttpError(413, `files are limited to ${gb(this.maxUpload)}`);
    if (r.declared > before.declared || r.peakEntries > before.peakEntries) {
  ...
  claimEntry(r) {
    ...
    r.peakEntries = Math.max(r.peakEntries, r.usedEntries);
    if (this.availableInodes() < this.inodeReserve) throw full();
    if (this.available() < this.reserveBytes) throw full();
    const err = this.overQuota(r.budget);
    if (err) throw err;
  ...
  async guard(budget, req, replacing, write, { entries = 0, peakEntries = entries } = {}) {
    const r = await this.reserve(budget, Number(req.headers['content-length']), replacing, entries, peakEntries);
    let ok = false;
    try {
  ...
  async withBytes(budget, bytes, replacing, fn, { entries = 0, peakEntries = entries } = {}) {
    const r = await this.reserve(budget, bytes, replacing, entries, peakEntries);
    let ok = false;
    try {
...
// Content bytes and entries under a path (links not followed), for sizing copies before they happen. Counted the way
// accounted() counts: each entry is charged ENTRY_COST separately, so folders add no content bytes.
export async function treeStats(p) {
  ...
  const total = { bytes: 0, entries: 1 }; // a folder's block is its entry charge (ENTRY_COST), not content
