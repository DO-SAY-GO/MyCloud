...
// Bytes and entries a directory tree accounts for. `skip` is a directory whose contents are not counted (the
// thumbnail cache is system data); the directory itself still is.
export async function accounted(root, skip = null) {
  let bytes = 0;
  let entries = 0;
