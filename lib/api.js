...
export class Api {
  constructor({ auth, store, thumbs, dataDir, signing, icon, limits, activity, drive }) {
    this.drive = drive;
  ...
  async handle(req, res, url, ip) {
    ...
    switch (route) {
      ...
      case 'PUT /files/raw': {
        ...
        await this.drive.put(user, p, req);
      ...
      case 'POST /files/mkdir': {
        ...
        await this.drive.mkdir(user, dirPath);
        return sendJson(res, 200, {});
      }
      case 'POST /files/move': {
        ...
        // The same path as WebDAV MOVE: budget and path locks, measured and billed, never replacing anything here.
        await this.drive.transfer({ user, move: true, src, dest: dst, destRel: splitPath(to).join('/'), overwrite: false }).catch((e) => {
          throw e.status === 412 ? new HttpError(409, 'something with that name already exists') : e;
        });
        return sendJson(res, 200, {});
      }
      case 'DELETE /files': {
        ...
        await this.drive.trash(user, this.filePath(user, rel), rel);
      ...
      case 'POST /trash/restore': {
        ...
        const restored = await this.drive.restore(user, id, !!shared);
      ...
      case 'POST /import': {
        ...
        await this.admit(user, dir, bytes, replacing,
          (res) => this.store.writeItems(dir, items, { settle: (x) => this.limits.settle(res, x) }),
          { entries, peakEntries: items.length });
