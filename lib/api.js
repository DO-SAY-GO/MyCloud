...
export class Api {
  ...
  async handle(req, res, url, ip) {
    ...
    if (route === 'POST /login') {
      ...
      if (this.auth.throttled(ip, u)) throw new HttpError(429, 'too many attempts, try again shortly');
      if (!(await this.auth.checkPassword(u, password))) {
        this.auth.noteFailure(ip, u);
        this.log('login.fail', req, { user: u });
        throw new HttpError(401, 'wrong username or password');
      }
      this.auth.noteSuccess(ip, u);
    ...
    switch (route) {
      ...
      case 'PUT /files/raw': {
        ...
        await this.limits.admit(user, Number(req.headers['content-length']));
        await fs.mkdir(path.dirname(p), { recursive: true });
        const tmp = path.join(path.dirname(p), `.mycloud-upload-${crypto.randomBytes(6).toString('hex')}`);
        try {
          await pipeline(req, this.limits.meter(user), createWriteStream(tmp));
      ...
      case 'POST /shares': {
        ...
        if (segs[0] === FAMILY_FOLDER && !admin) throw new HttpError(403, 'only the family admin can publish things from the Family folder');
