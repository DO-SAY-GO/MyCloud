...
export class Api {
  ...
  async handle(req, res, url, ip) {
    ...
    switch (route) {
      ...
      case 'PUT /files/raw': {
        ...
        try {
          const old = await statOrNull(p);
          await this.limits.guard(await this.limits.budgetFor(user, p), req, old?.size ?? 0, async (meter, res) => {
            await pipeline(req, meter, createWriteStream(tmp));
            // Settle against what is there *now* (a concurrent upload may have created or replaced it), then swap.
            await this.store.withLock(p, async () => {
              const now = await statOrNull(p);
      ...
      case 'POST /files/mkdir': {
        ...
        await this.admit(user, dirPath, 4096 * splitPath(rel).length, 0, () => fs.mkdir(dirPath, { recursive: true }), { entries: splitPath(rel).length });
        return sendJson(res, 200, {});
      }
      case 'POST /files/move': {
        ...
        if ((await this.limits.budgetFor(user, src)) === (await this.limits.budgetFor(user, path.dirname(dst)))) await fs.rename(src, dst);
          ...
          const t = await treeStats(src);
          const from = await this.limits.budgetFor(user, src);
          await this.admit(user, path.dirname(dst), t.bytes, 0, () => fs.rename(src, dst), { entries: t.entries });
          this.limits.freed(from, { bytes: t.bytes + t.entries * ENTRY_COST, entries: t.entries }); // left `from`
        }
        return sendJson(res, 200, {});
      }
      case 'DELETE /files': {
      ...
      case 'POST /events': {
        ...
        await this.admit(user, dir, Buffer.byteLength(ics), 0, () => this.store.writeItem(dir, `${id}.ics`, ics), { entries: 1 });
      ...
      case 'POST /contacts': {
        ...
        await this.admit(user, dir, Buffer.byteLength(vcf), 0, () => this.store.writeItem(dir, `${id}.vcf`, vcf), { entries: 1 });
      ...
      case 'POST /import': {
        ...
        for (const [name, data] of items) {
          ...
          const old = await statOrNull(path.join(dir, name));
          replacing += old?.size ?? 0;
          if (!old) entries++;
        }
        await this.admit(user, dir, bytes, replacing, (res) => this.store.writeItems(dir, items, { settle: (x) => this.limits.settle(res, x) }), { entries });
  ...
  async newCollection(user, area, name) {
    ...
    await this.admit(user, this.store.collectionsRoot(user, area), 0, 0, () => this.store.createCollection(user, area, id, { '{DAV:}displayname': name.slice(0, 100) }), { entries: 1 });
    return id;
  }
