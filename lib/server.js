...
export async function createServer({ dataDir, cert, key, trustProxy = false, secureCookies, log = console }) {
  ...
  const signCert = process.env.MYCLOUD_SIGN_CERT || cert;
  const signKey = process.env.MYCLOUD_SIGN_KEY || key;
  const signing = signCert && signKey ? { cert: signCert, key: signKey, chain: process.env.MYCLOUD_SIGN_CHAIN } : null;
  const icon = await fs.readFile(path.join(PUBLIC_DIR, 'icon-180.png')).catch(() => null);
  const api = new Api({ auth, store, thumbs: new Thumbnailer(), dataDir, secureCookies: secureCookies ?? !!cert, signing, icon });
  await api.load();
