...
export async function createServer({ dataDir, cert, key, trustProxy = false, publicUrl, log = console, env = process.env }) {
  ...
  const limits = new Limits(store, env);
  const dav = new Dav(store, { limits, activity });
  ...
  const thumbs = new Thumbnailer({ dataDir, log, hidePaths: [cert, key, signCert, signKey, env.MYCLOUD_SIGN_CHAIN],
    admit: (bytes, fn) => limits.withBytes(SYSTEM, bytes, 0, fn) }); // cache is nobody's quota, but respects the disk reserve
  const api = new Api({ auth, store, thumbs, dataDir, signing, icon, limits, activity });
  await api.load();
  ...
  return { server, auth, store, activity };
}
