  ...
  async transfer({ user, move, src, dest, destRel, overwrite }) {
    ...
    if (move) {
      const run = async (res) => {
        ...
        // A folder move holds both budgets exclusively: nothing is written beneath it while it is measured and moved.
        await store.withBudgets([src, dest], srcStat.isDirectory(), () => store.withLocks([src, dest], async () => {
      ...
      await limits.withBytes(to, estimate.bytes, 0, async (res) => {
        try {
          ...
          limits.settle(res, { entries: res.usedEntries ?? 0 });
